...
// La lógica de captura es la MISMA que el hook anterior: template dinámico por
// commodity + payload {metrics:{ "family.code": valor }, photos:{ key:[urls] }} → POST /api/inspecciones.
...
const GROUPS = {
  quality:     { label: 'Calidad',   icon: 'flask' },
  condition:   { label: 'Condición', icon: 'thermometer' },
  packaging:   { label: 'Embalaje',  icon: 'package' },
  measurement: { label: 'Medición',  icon: 'ruler' },
  _other:      { label: 'Otros',     icon: 'list' },
}
    ...
    <header className="insp-top">
      <div className="insp-brand">
        ...
        <div>
          ...
          <div className="brand-sub">Inspección de campo</div>
        </div>
      </div>
      <div className="insp-nav-center">
        <div className="seg">
          <button className={tab === 'asignadas' ? 'on' : ''} onClick={() => onTab('asignadas')}>
            <Icon name="clipboard" size={15} />Asignadas{assignedCount > 0 && <span className="cnt">{assignedCount}</span>}
          </button>
          <button className={tab === 'completadas' ? 'on' : ''} onClick={() => onTab('completadas')}>
            <Icon name="clipboardCheck" size={15} />Completadas
          </button>
        </div>
      </div>
      <div className="insp-top-right" ref={ref}>
        <button className="btn btn-icon" title="Cambiar tema" onClick={onTheme}><Icon name={theme === 'dark' ? 'sun' : 'moon'} size={18} /></button>
        <button className="avatar" style={{ border: 0, cursor: 'pointer' }} onClick={() => setMenu(o => !o)}>{initial}</button>
        {menu && (
          <div className="menu-pop">
            <div className="menu-head">
              <div style={{ fontSize: 13, fontWeight: 600 }}>{user?.name || 'Inspector'}</div>
              <div style={{ fontSize: 11.5, color: 'var(--text-faint)', marginTop: 1 }}>{user?.email || ''}</div>
            </div>
            {user?.role === 'admin' && <a className="menu-item" href="/admin"><Icon name="dashboard" size={16} />Vista administrador</a>}
            <button className="menu-item danger" onClick={onLogout}><Icon name="logout" size={16} />Cerrar sesión</button>
          </div>
        )}
      </div>
    </header>
...
function QueueScreen({ assigned, loading, onStart }) {
  if (loading) return <div className="insp-inner"><section className="card"><div className="empty" style={{ padding: 64 }}><Icon name="clock" size={22} /> Cargando asignaciones…</div></section></div>
  return (
    <div className="insp-inner fade-up">
      <div className="page-h">
        <h1><Icon name="clipboard" size={22} style={{ color: 'var(--accent-strong)' }} />Mis inspecciones asignadas</h1>
        <span className="sub">{assigned.length} {assigned.length === 1 ? 'pendiente' : 'pendientes'}</span>
      </div>
      {assigned.length === 0 ? (
        <section className="card">
          <div className="empty" style={{ padding: '72px 20px' }}>
            ...
            <div style={{ fontSize: 16, fontWeight: 600, color: 'var(--text)', marginBottom: 6 }}>Todo al día</div>
            <div style={{ maxWidth: 340, margin: '0 auto', lineHeight: 1.55 }}>No tienes inspecciones pendientes. Cuando el administrador te asigne una, aparecerá aquí.</div>
          </div>
        </section>
      ) : (
        <div className="queue-grid stagger">
          {assigned.map(t => {
            const c = commodityVisual(t.commodity_code)
            return (
              <section className="card task-card" key={t.id}>
                <div className="task-top">
                  <div className={'commodity-ico ' + c.key}><Icon name={c.icon} size={22} /></div>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div className="task-lote">{t.lote}</div>
                    <div className="task-prod">{t.productor}</div>
                  </div>
                </div>
                <div className="task-meta">
                  <span className="m"><Icon name="grape" size={13} />{c.label} · {t.variedad}</span>
                  <span style={{ color: 'var(--border-strong)' }}>·</span>
                  <span className="m"><Icon name="calendar" size={13} />Asignada {t.asignada}</span>
                </div>
                {t.notasAdmin && (
                  <div style={{ fontSize: 12, color: 'var(--text-dim)', background: 'var(--surface-2)', border: '1px solid var(--border)', borderRadius: 9, padding: '8px 10px', margin: '0 0 12px', lineHeight: 1.5 }}>
                    <b style={{ color: 'var(--text)' }}>Nota:</b> {t.notasAdmin}
                  </div>
                )}
                <button className="btn btn-primary" style={{ width: '100%' }} onClick={() => onStart(t)}>
                  <Icon name="play" size={16} stroke={2} />Iniciar inspección
                </button>
              </section>
            )
          })}
...
function CompletedScreen({ completed, loading, onOpen }) {
  if (loading) return <div className="insp-inner"><section className="card"><div className="empty" style={{ padding: 64 }}><Icon name="clock" size={22} /> Cargando…</div></section></div>
  return (
    <div className="insp-inner fade-up">
      <div className="page-h">
        <h1><Icon name="clipboardCheck" size={22} style={{ color: 'var(--green)' }} />Inspecciones completadas</h1>
        <span className="sub">{completed.length} finalizadas</span>
      </div>
      <section className="card card-pad">
        {completed.length === 0 ? (
          <div className="empty" style={{ padding: 56 }}>
            <div className="ei"><Icon name="clipboardCheck" size={26} /></div>
            <div style={{ fontSize: 15, fontWeight: 600, color: 'var(--text)', marginBottom: 6 }}>Aún no completas inspecciones</div>
            <div>Las inspecciones que registres aparecerán aquí.</div>
          </div>
        ) : (
          <table className="tbl">
            <thead><tr>
              <th>Fecha</th><th>Lote / productor</th><th>Commodity</th><th className="num">Score</th><th>Resolución</th><th></th>
            </tr></thead>
            <tbody>
              {completed.map(i => {
...
function CompletedDrawer({ item, onClose }) {
  ...
  const fmtv = (m) => m.value_num != null ? String(m.value_num) : (m.option_value ?? (m.value_bool != null ? (m.value_bool ? 'Sí' : 'No') : (m.value_text ?? '—')))
    ...
    <>
      ...
      <aside className="drawer">
        ...
        <div className="drawer-body">
          {loading && <div className="empty" style={{ padding: 48 }}><Icon name="clock" size={20} /> Cargando detalle…</div>}
          ...
          {!loading && !err && detail && (() => {
            ...
            const physChem = [
              ['Brix prom.', detail.brix_avg, '°Bx'],
              ['Temp. pulpa', detail.temp_pulp, '°C'],
              ['Temp. ambiente', detail.temp_ambient, '°C'],
              ['Temp. agua', detail.temp_water, '°C'],
              ['Calibre', detail.diameter_min != null || detail.diameter_max != null ? `${detail.diameter_min ?? '—'}–${detail.diameter_max ?? '—'}` : null, 'mm'],
              ['Peso neto', detail.net_weight, 'kg'],
            ].filter(([, v]) => v != null && v !== '')
            return (
              <>
                <div className="score-hero" style={{ marginBottom: 22 }}>
                  ...
                  <div>
                    ...
                    <div className={'badge ' + r.color} style={{ height: 30, fontSize: 14, padding: '0 14px' }}><Icon name={r.icon} size={17} stroke={2} />{r.label}</div>
                    {detail.total_defects_pct != null && (
                      <div style={{ fontSize: 12, color: 'var(--text-faint)', marginTop: 10, lineHeight: 1.5 }}>
                        {fmt1(detail.total_defects_pct)}% defectos totales<br />
                        Calidad {fmt1(detail.quality_total_pct)}% · Condición {fmt1(detail.condition_total_pct)}%
                      </div>
                    )}
                  </div>
                </div>
                ...
                {physChem.length > 0 && (<>
                  <div className="sec-title" style={{ marginBottom: 12 }}>Mediciones físico-químicas</div>
                ...
                <div className="sec-title" style={{ marginBottom: 14 }}>Métricas de calidad</div>
                {measurements.length === 0 ? (
                  <div style={{ fontSize: 13, color: 'var(--text-faint)' }}>Sin métricas registradas.</div>
                ) : measurements.map(m => (
                ...
                {detail.notes && (
                  <div style={{ marginTop: 22 }}>
                    <div className="sec-title" style={{ marginBottom: 10 }}>Observaciones</div>
        ...
        <div className="drawer-foot">
          ...
          {item.pdfUrl
            ? <a className="btn btn-primary" style={{ flex: 1, textAlign: 'center' }} href={item.pdfUrl} target="_blank" rel="noreferrer"><Icon name="download" size={16} />Ver PDF</a>
            : <button className="btn btn-primary" style={{ flex: 1 }} disabled><Icon name="download" size={16} />PDF no generado</button>}
        </div>
      </aside>
    </>
  )
  ...
  const onPick = async (e) => {
    ...
    try {
      ...
      for (const f of files.slice(0, 3 - urls.length)) {
        if (!f.type.startsWith('image/')) throw new Error('Archivo no es imagen')
        if (f.size > 5 * 1024 * 1024) throw new Error('Imagen > 5MB')
      ...
      onChange([...urls, ...out])
    } catch (e2) { setErr(e2.message || 'Error al subir') }
    ...
    <div>
        ...
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginTop: 8 }}>
          {urls.map((u, i) => (
            <div key={i} style={{ position: 'relative', width: 46, height: 46 }}>
              ...
              <button type="button" className="rm" onClick={() => onChange(urls.filter((_, j) => j !== i))} aria-label="quitar" style={{ width: 16, height: 16 }}><Icon name="x" size={10} /></button>
            </div>
          ))}
        </div>
      )}
      {urls.length < 3 && (
        <label className="btn btn-sm" style={{ marginTop: 8, cursor: busy ? 'wait' : 'pointer' }}>
          ...
          <Icon name="camera" size={14} />{busy ? 'Subiendo…' : `Foto (${urls.length}/3)`}
        </label>
      )}
...
/* ───────────────── captura ───────────────── */
const EMPTY_HEADER = {
  caliber: '', packaging_code: '', packaging_type: '', packaging_date: '',
...
function CaptureForm({ task, onSave, onCancel, onError }) {
  ...
  useEffect(() => {
    ...
    getTemplate(task.commodity_code)
      ...
      .catch(e => onError(`No se pudo cargar la plantilla de ${c.label}: ${e.message}`))
  ...
  const save = async () => {
    ...
    try {
      ...
      onError(e.message || 'Error al guardar la inspección')
    } finally {
      setSaving(false)
    }
  }
    ...
    return (
      <div key={f.key}>
        <label className="field-label" style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          {f.is_major && <span title="Mayor" style={{ width: 6, height: 6, borderRadius: '50%', background: 'var(--red)' }} />}{label}
        </label>
        {type === 'select' ? (
          ...
          <div className="seg" style={{ width: '100%' }}>
            <button type="button" className={values[f.key] === 'true' ? 'on' : ''} style={{ flex: 1 }} onClick={() => setV(f.key, 'true')}>Sí</button>
            <button type="button" className={values[f.key] === 'false' ? 'on' : ''} style={{ flex: 1 }} onClick={() => setV(f.key, 'false')}>No</button>
          </div>
        ) : type === 'text' ? (
  ...
  return (
    <div className="insp-inner wide fade-up">
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20 }}>
        <button className="btn btn-sm" onClick={onCancel}><Icon name="chevLeft" size={16} />Volver</button>
        ...
        <span className="badge neutral" style={{ marginLeft: 'auto' }}><Icon name="clipboardCheck" size={13} />Captura en curso</span>
      </div>

      <div className="capture-grid">
        <div className="capture-col">
          <SectionCard n="1" icon="package" title="Embalaje" sub="Identificación del envase y peso">
            <div className="form-grid">
              <div>
                <label className="field-label">Código de embalaje</label>
                <input className="input mono" placeholder="EMB-0000" value={header.packaging_code} onChange={e => setH('packaging_code', e.target.value)} />
              </div>
              <div>
                ...
                <select className="select" value={header.packaging_type} onChange={e => setH('packaging_type', e.target.value)}>
                  <option value="">Seleccionar…</option>
                  {TIPOS_EMBALAJE.map(t => <option key={t}>{t}</option>)}
                </select>
              </div>
              <div>
                <label className="field-label">Fecha de embalaje</label>
          ...
          <SectionCard n="2" icon="thermometer" title="Mediciones físico-químicas" sub="Brix, temperatura y calibre">
            <div className="measure-group">
              <div className="measure-group-h"><Icon name="droplet" size={15} style={{ color: 'var(--accent-strong)' }} />Brix</div>
              <div className="measure-cols" style={{ gridTemplateColumns: '1fr' }}>
                <div><label className="field-label">Promedio</label><UnitField value={header.brix_avg} onChange={v => setH('brix_avg', v)} unit="°Bx" /></div>
              </div>
            </div>
            <div className="measure-group">
              <div className="measure-group-h"><Icon name="thermometer" size={15} style={{ color: 'var(--accent-strong)' }} />Temperatura (°C)</div>
              <div className="measure-cols" style={{ gridTemplateColumns: '1fr 1fr 1fr' }}>
                <div><label className="field-label">Agua</label><UnitField value={header.temp_water} onChange={v => setH('temp_water', v)} unit="°C" /></div>
                <div><label className="field-label">Ambiente</label><UnitField value={header.temp_ambient} onChange={v => setH('temp_ambient', v)} unit="°C" /></div>
                <div><label className="field-label">Pulpa</label><UnitField value={header.temp_pulp} onChange={v => setH('temp_pulp', v)} unit="°C" /></div>
              </div>
            </div>
            <div className="measure-group">
              <div className="measure-group-h"><Icon name="ruler" size={15} style={{ color: 'var(--accent-strong)' }} />Calibre / diámetro</div>
              <div className="measure-cols" style={{ gridTemplateColumns: '1fr' }}>
                <div><label className="field-label">Rango (mm)</label><input className="input mono" placeholder="12–18" value={header.caliber} onChange={e => setH('caliber', e.target.value)} /></div>
              </div>
            </div>
          </SectionCard>

          <SectionCard n="3" icon="list" title="Muestreo y defectos" sub={`Evaluación de ${c.label.toLowerCase()} según la plantilla`}>
            {loadingTpl ? (
              <div className="empty" style={{ padding: 40 }}><Icon name="clock" size={18} /> Cargando plantilla…</div>
            ) : fields.length === 0 ? (
              <div className="empty" style={{ padding: 32 }}>Esta commodity no tiene plantilla de defectos.</div>
            ) : (
              Object.entries(grouped).map(([fam, fs]) => {
                const g = GROUPS[fam] || GROUPS._other
                return (
                  <div className="measure-group" key={fam}>
                    <div className="measure-group-h"><Icon name={g.icon} size={15} style={{ color: 'var(--accent-strong)' }} />{g.label}</div>
          ...
          <SectionCard n="4" icon="edit" title="Observaciones" sub="Notas del inspector">
            <textarea className="textarea" rows={3} placeholder="Condición general, cadena de frío, acciones recomendadas…" value={header.notes} onChange={e => setH('notes', e.target.value)} />
          </SectionCard>
        </div>
        ...
        <div className="capture-summary">
          <section className="card card-pad">
            ...
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12, padding: '6px 0 18px', borderBottom: '1px solid var(--border)', marginBottom: 14 }}>
              <Donut segments={[{ value: progress, varName: '--accent' }, { value: 100 - progress, varName: '--track' }]} size={120} thickness={14} centerTop={`${filled}/${fields.length}`} centerBottom="CAMPOS" />
              <div style={{ fontSize: 11.5, color: 'var(--text-faint)', textAlign: 'center', lineHeight: 1.5 }}>El <b>score</b> y la resolución se calculan en el servidor al guardar.</div>
            </div>

            <div className="sum-stat"><span className="k"><Icon name="list" size={14} />Métricas</span><span className="v">{filled}/{fields.length}</span></div>
            <div className="sum-stat"><span className="k"><Icon name="droplet" size={14} />Brix prom.</span><span className="v">{header.brix_avg || '—'}</span></div>
            <div className="sum-stat"><span className="k"><Icon name="camera" size={14} />Fotos</span><span className="v">{photoCount}</span></div>
            ...
            <div className="capture-actions" style={{ flexDirection: 'column', marginTop: 18 }}>
              <button className="btn btn-primary" style={{ width: '100%' }} onClick={save} disabled={saving || loadingTpl}>
                <Icon name="check" size={17} stroke={2.4} />{saving ? 'Guardando…' : 'Guardar inspección'}
              </button>
              <button className="btn" style={{ width: '100%' }} onClick={onCancel} disabled={saving}>Cancelar</button>
            </div>
          </section>
        </div>
      </div>
...
export default function InspectorApp() {
  ...
  const showToast = useCallback((t) => { setToast(t); setTimeout(() => setToast(null), 3400) }, [])

  const loadAssigned = useCallback(() => {
    setLoadingA(true)
    getAssigned().then(setAssigned).catch(e => showToast({ title: 'Error', sub: e.message, bad: true })).finally(() => setLoadingA(false))
  }, [showToast])
  const loadCompleted = useCallback(() => {
    setLoadingC(true)
    getCompleted().then(setCompleted).catch(e => showToast({ title: 'Error', sub: e.message, bad: true })).finally(() => setLoadingC(false))
  }, [showToast])
  ...
  const onSaved = (record) => {
    ...
    const res = record.resolucion ? (RES[{ approved: 'aprobado', conditional: 'condicional', rejected: 'rechazado' }[record.resolucion]]?.label || record.resolucion) : null
    showToast({ title: 'Inspección registrada', sub: `${record.lote}${res ? ' · ' + res : ''}` })
  }

  return (
    <div className="insp-shell">
      {!capture && <InspTopBar tab={tab} onTab={setTab} assignedCount={assigned.length} user={user} theme={theme} onTheme={() => setTheme(t => t === 'dark' ? 'light' : 'dark')} onLogout={logout} />}
      <div className="insp-content">
        {capture
          ? <CaptureForm task={capture} onSave={onSaved} onCancel={() => setCapture(null)} onError={(m) => showToast({ title: 'Error', sub: m, bad: true })} />
          : tab === 'asignadas'
