...
const HEAD_KEYS = ['producer', 'variety', 'brix_avg', 'brix_min', 'brix_max', 'diameter_min', 'diameter_max', 'temp_water', 'temp_ambient', 'temp_pulp', 'net_weight', 'sample_weight_g', 'ten_pieces_weight_g', 'notes']
  ...
  const submit = async () => {
    ...
    try {
      ...
      await updateInspeccion(detail.id, {
        ...
        net_weight: numOrNull(head.net_weight), sample_weight_g: numOrNull(head.sample_weight_g), ten_pieces_weight_g: numOrNull(head.ten_pieces_weight_g),
  ...
  return (
    <Modal size="lg" title={`${t('edit.title')} #${detail.id}`} icon="edit" onClose={onClose}
      ...
      <div className="form-grid">
        ...
        <Field label={t('ni.tenPieces')}><input className="input mono" inputMode="decimal" value={head.ten_pieces_weight_g} onChange={e => setH('ten_pieces_weight_g', e.target.value)} /></Field>
      </div>
      <div className="form-grid">
      </div>
      <div className="form-grid">
