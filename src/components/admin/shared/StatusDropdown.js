...
const STATUS_ICONS = {
  pendiente:  (color) => <Circle      size={11} fill={color} color={color} />,
  completada: (color) => <CheckCircle2 size={11} color={color} />,
  cancelada:  (color) => <XCircle     size={11} color={color} />
}
...
export default function StatusDropdown({ assignmentId, currentStatus, onChanged }) {
  ...
  return (
    <div style={{ position: 'relative', display: 'inline-block' }}>
      ...
      >
        {saving ? '…' : <>{STATUS_ICONS[currentStatus]?.(cfg.color)} {cfg.label}</>}
        ...
        <>
          ...
          <div style={{
            ...
            {STATUS_OPTIONS.map(opt => (
              ...
              >
                {STATUS_ICONS[opt.value]?.(opt.color)}
                {opt.label}
              </button>
            ))}
          </div>
