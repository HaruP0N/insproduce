...
export default function AsignacionesEnCurso({ asignaciones, onChanged }) {
  ...
  return (
    <div style={S.card}>
      <div style={{ ...S.cardHead, display: 'flex', alignItems: 'center', gap: 8 }}>
        <Inbox size={15} color="#15803d" />
      ...
      {activas.length === 0 ? (
        <div style={{ padding: 40, textAlign: 'center', color: '#9ca3af' }}>
          ...
          <div style={{ fontSize: 16, fontWeight: 700, marginBottom: 8, color: '#374151' }}>No hay asignaciones activas</div>
          <div style={{ fontSize: 13 }}>Las asignaciones pendientes y en proceso aparecerán aquí.</div>
        </div>
      ) : (
        <table style={S.tbl}>
          <thead>
          ...
          <tbody>
            {activas.map(a => (
              <tr key={a.id}>
                ...
                <td style={S.td}>
                  <StatusDropdown assignmentId={a.id} currentStatus={a.status || 'pendiente'} onChanged={onChanged} />
                </td>
              </tr>
            ))}
          </tbody>
