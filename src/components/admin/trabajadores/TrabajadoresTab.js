...
import { Users, Crown, HardHat, CheckCircle2, XCircle, Plus, Pencil, Ban, X } from 'lucide-react'
...
export default function TrabajadoresTab() {
  ...
  const fetchUsers = async () => {
    ...
    try {
      ...
      setError('Error: ' + err.message); setUsuarios([])
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleCreate = async (e) => {
    ...
    try {
      ...
      setCreateOpen(false); setNewUser({ name: '', email: '', password: '', role: 'inspector' }); fetchUsers()
  ...
  if (error)   return <div style={{ padding: 24 }}><div style={{ padding: 14, background: '#fef2f2', borderRadius: 12, color: '#dc2626' }}>{error}</div></div>
    ...
    <div style={{ padding: 18, maxWidth: 1240, margin: '0 auto' }}>
      <div style={{ ...S.card, padding: 20, overflow: 'visible' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
          <div>
            <h2 style={{ margin: 0, color: '#15803d', fontSize: 20, fontWeight: 900, display: 'flex', alignItems: 'center', gap: 8 }}>
              <Users size={20} /> Gestión de Trabajadores
            </h2>
            <p style={{ margin: '4px 0 0', color: '#6b7280', fontSize: 13 }}>{usuarios.length} usuarios</p>
          </div>
          <Button onClick={() => setCreateOpen(true)}><Plus size={15} /> Crear usuario</Button>
        </div>
          ...
          <tbody>
            {usuarios.map(u => (
              <tr key={u.id}>
                ...
                <td style={S.td}>
                  <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, fontWeight: 700, color: u.role === 'admin' ? '#b45309' : '#15803d' }}>
                    {u.role === 'admin' ? <Crown size={13} /> : <HardHat size={13} />}
                    {u.role === 'admin' ? 'Admin' : 'Inspector'}
                  </span>
                </td>
                <td style={S.td}>
                  <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, padding: '4px 10px', borderRadius: 999, fontSize: 11, fontWeight: 700, background: u.active ? '#f0fdf4' : '#fef2f2', color: u.active ? '#166534' : '#dc2626', border: `1px solid ${u.active ? '#86efac' : '#fca5a5'}` }}>
                ...
                <td style={S.td}>
                  <div style={{ display: 'flex', gap: 8 }}>
                    <Button variant="outline" small onClick={() => { setEditUser(u); setEditForm({ name: u.name || '', email: u.email || '', role: u.role || '', password: '' }); setEditOpen(true) }}>
                      <Pencil size={12} /> Editar
                    </Button>
                    <Button variant={u.active ? 'gray' : 'primary'} small onClick={() => toggleActive(u)}>
                      <Ban size={12} /> {u.active ? 'Desactivar' : 'Activar'}
                    </Button>
                  </div>
                </td>
              </tr>
        ...
        <div style={S.modalOv} onClick={e => e.target === e.currentTarget && setCreateOpen(false)}>
          <div style={S.modalSm}>
            <div style={S.modalHead}>
              ...
              <Button variant="gray" small onClick={() => setCreateOpen(false)}><X size={13} /></Button>
            </div>
              ...
              <div style={S.modalBody}>
                {[['name','text','Nombre completo'],['email','email','Email *'],['password','password','Contraseña * (mín. 6 caracteres)']].map(([k,t,lbl]) => (
                  <div key={k} style={{ marginBottom: 12 }}>
                    ...
                    <input style={S.field} type={t} value={newUser[k]} onChange={e => setNewUser(p => ({ ...p, [k]: e.target.value }))} required={k !== 'name'} minLength={k === 'password' ? 6 : undefined} />
                  </div>
                ))}
        ...
        <div style={S.modalOv} onClick={e => e.target === e.currentTarget && setEditOpen(false)}>
          <div style={S.modalSm}>
            <div style={S.modalHead}>
              ...
              <Button variant="gray" small onClick={() => setEditOpen(false)}><X size={13} /></Button>
            </div>
            <form onSubmit={handleEdit}>
              <div style={S.modalBody}>
                {[['name','text','Nombre'],['email','email','Email'],['password','password','Nueva contraseña (opcional)']].map(([k,t,lbl]) => (
                  <div key={k} style={{ marginBottom: 12 }}>
                    <label style={S.label}>{lbl}</label>
                    <input style={S.field} type={t} value={editForm[k]} onChange={e => setEditForm(p => ({ ...p, [k]: e.target.value }))} placeholder={k === 'password' ? 'Dejar vacío para no cambiar' : undefined} />
                  </div>
                ))}
                <div style={{ marginBottom: 12 }}>
                  <label style={S.label}>Rol</label>
