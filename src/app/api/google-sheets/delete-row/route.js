...
export async function DELETE(req) {
  ...
  try {
    ...
    if (!body.rowNumber)
      return NextResponse.json({ msg: 'rowNumber es obligatorio' }, { status: 400 })

    await sheetsClient.writeSheet(
      spreadsheetId,
      `A${body.rowNumber}:G${body.rowNumber}`,
      [['', '', '', '', '', '', '']]
    )

    return NextResponse.json({ msg: 'Fila eliminada' })
  } catch (e) {
    console.error('[google-sheets/delete-row]', e)
    return NextResponse.json({ msg: 'Error al eliminar: ' + e.message }, { status: 500 })
  }
}